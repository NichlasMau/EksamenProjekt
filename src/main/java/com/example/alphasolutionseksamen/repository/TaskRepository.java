...
public class TaskRepository {
    ...
    public List < Task > getCustomerTasks(int givenCustomerId) {
        List < Task > tasks = new ArrayList < > ();
        try (Connection con = DBConnector.getConnection()) {
            String SQL = "SELECT * FROM tasks t JOIN subprojects sp ON t.subproject_id = sp.subproject_id JOIN projects p ON sp.project_id = p.project_id JOIN project_customers pc ON p.project_id = pc.project_id WHERE pc.customer_id = ?;";
            PreparedStatement pstmt = con.prepareStatement(SQL);
    ...
    public int getProjectIdByTaskId(int subprojectId) {
        try (Connection con = connector.getConnection()) {
            String SQL = "SELECT project_id FROM subprojects WHERE subproject_id = ?";
            PreparedStatement pstmt = con.prepareStatement(SQL);
    ...
    public int getSubprojectBudgetUsed(int subprojectId) {
        try (Connection con = connector.getConnection()) {
            String SQL = "SELECT SUM(budget) AS total_budget FROM tasks WHERE subproject_id = ? AND status = 'done';";
            PreparedStatement pstmt = con.prepareStatement(SQL);
            pstmt.setInt(1, subprojectId);
            ResultSet rs = pstmt.executeQuery();
    ...
    public int getSubprojectBudget(int subprojectId) {
        try (Connection con = connector.getConnection()) {
            String SQL = "SELECT budget FROM subprojects WHERE subproject_id = ?;";
            PreparedStatement pstmt = con.prepareStatement(SQL);
            pstmt.setInt(1, subprojectId);
            ResultSet rs = pstmt.executeQuery();
    ...
    public int getTotalEstimatedTime(int subprojectId) {
        try (Connection con = connector.getConnection()) {
            String SQL = "SELECT SUM(estimated_time) AS total_estimated_time FROM tasks WHERE subproject_id = ?;";
            PreparedStatement pstmt = con.prepareStatement(SQL);
            pstmt.setInt(1, subprojectId);
            ResultSet rs = pstmt.executeQuery();
            ...
            if (rs.next()) {
                return rs.getInt("total_estimated_time");
            } else {
                return -1;
            }
        } catch (SQLException e) {
    ...
    public int getTotalEstimatedTimeFinished(int subprojectId) {
        try (Connection con = connector.getConnection()) {
            String SQL = "SELECT SUM(estimated_time) AS total_estimated_time FROM tasks WHERE subproject_id = ? AND status = 'done';";
            PreparedStatement pstmt = con.prepareStatement(SQL);
            pstmt.setInt(1, subprojectId);
            ResultSet rs = pstmt.executeQuery();
